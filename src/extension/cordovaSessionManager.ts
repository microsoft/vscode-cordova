...
import {
  CordovaSession,
  CordovaSessionStatus,
} from "../debugger/debugSessionWrapper";

export class CordovaSessionManager
  implements vscode.DebugAdapterDescriptorFactory
{
  ...
  public terminate(
    ...
    if (restart && this.cordovaDebugSessions.has(cordovaDebugSessionId)) {
      this.restartingVSCodeSessions.add(
        this.cordovaDebugSessions
          .get(cordovaDebugSessionId)
    ...
    this.destroyServer(
      cordovaDebugSessionId,
      this.servers.get(cordovaDebugSessionId)
    );
  ...
  public getCordovaDebugSessionByProjectRoot(
    projectRoot: string
  ): CordovaSession | null {
    for (let cordovaSession of this.cordovaDebugSessions.values()) {
      if (
        cordovaSession.getVSCodeDebugSession().workspaceFolder.uri.fsPath ===
        projectRoot
      ) {
