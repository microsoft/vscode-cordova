...
export class SafariIonicCDPMessageHandler extends SafariCDPMessageHandlerBase {
  ...
  public configureHandlerAfterAttachmentPreparation(
    options: HandlerOptions
  ): void {
  ...
  public processApplicationCDPMessage(event: any): ProcessedCDPMessage {
    ...
    if (
      ...
      event.params.url &&
      (event.params.url.startsWith(
        `ionic://${this.applicationServerAddress}`
      ) ||
  ...
  private fixIonic3RuntimeEvaluateErrorResponse(event: any) {
    if (
      event.result.result &&
      event.result.result.value === this.Ionic3EvaluateErrorMessage
    ) {
