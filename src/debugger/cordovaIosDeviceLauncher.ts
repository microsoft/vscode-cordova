...
export class CordovaIosDeviceLauncher {
  ...
  public static getBundleIdentifier(projectRoot: string): Promise<string> {
    return fs.promises
      ...
      .then((files: string[]) => {
        let xcodeprojfiles = files.filter((file: string) =>
          /\.xcodeproj$/.test(file)
        );
  ...
  public static startDebugProxy(
    ...
    return CordovaIosDeviceLauncher.mountDeveloperImage().then(() => {
      return new Promise((resolve, reject) => {
        CordovaIosDeviceLauncher.nativeDebuggerProxyInstance =
          child_process.spawn("idevicedebugserverproxy", [
            proxyPort.toString(),
          ]);
  ...
  public static startWebkitDebugProxy(
    ...
    return new Promise(async (resolve, reject) => {
      let iwdpArgs = [
        "-c",
        `null:${proxyPort},:${proxyRangeStart}-${proxyRangeEnd}`,
      ];

      if (iOSTarget.isVirtualTarget) {
        const webInspectorSocketPath =
          await CordovaIosDeviceLauncher.getWebInspectorSocket(iOSTarget.id);
  ...
  private static getBundleIdentifierFromPbxproj(
    ...
    const bundleId =
      allConfigs[targetConfigUUID].buildSettings.PRODUCT_BUNDLE_IDENTIFIER;
    return Promise.resolve(bundleId);
  }

  private static async getWebInspectorSocket(
    simId: string
  ): Promise<string | null> {
    const cp = new ChildProcess();
  ...
  private static mountDeveloperImage(): Promise<void> {
    return CordovaIosDeviceLauncher.getDiskImage().then((path: string) => {
      let imagemounter: child_process.ChildProcess = child_process.spawn(
        "ideviceimagemounter",
        [path, path + ".signature"]
      );
      return new Promise((resolve, reject) => {
        ...
        imagemounter.on("close", function (code: number): void {
          if (code !== 0) {
            if (stdout.indexOf("Error:") !== -1) {
              ...
              stdout.indexOf("No device found, is it plugged in?") !== -1
            ) {
              reject(
                localize(
                  "UnableToFindDevice",
  ...
  private static getDiskImage(): Promise<string> {
    ...
    let versionInfo: Promise<string> = cp
    ...
    let pathInfo: Promise<string> = cp
    ...
    return Promise.all([versionInfo, pathInfo]).then(([version, sdkpath]) => {
      let find: child_process.ChildProcess = child_process.spawn("find", [
        sdkpath,
        "-path",
        "*" + version + "*",
        "-name",
