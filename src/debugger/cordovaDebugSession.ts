...
import {
  ...
  ProjectType,
} from "../utils/cordovaProjectHelper";
...
import {
  ...
  AndroidTarget,
} from "../utils/android/androidTargetManager";
...
export type DebugConsoleLogger = (
  message: string,
...
export class CordovaDebugSession extends LoggingDebugSession {
      ...
      return this.initializeTelemetry(launchArgs.cwd)
        ...
        .then(() =>
          TelemetryHelper.generate("launch", (generator) => {
            ...
            let platform =
              launchArgs.platform && launchArgs.platform.toLowerCase();
  ...
  protected doAttach(attachArgs: ICordovaAttachRequestArgs): Promise<void> {
    return new Promise<void>((resolve, reject) =>
      this.initializeTelemetry(attachArgs.cwd)
        .then(() => {
        ...
        .then(() =>
          TelemetryHelper.generate("attach", (generator) => {
            ...
            let platform =
              attachArgs.platform && attachArgs.platform.toLowerCase();
            ...
            return TelemetryHelper.determineProjectTypes(attachArgs.cwd)
              .then((projectType) => {
                let sourcemapPathTransformer = new SourcemapPathTransformer(
                  attachArgs,
                  projectType
                );
  ...
  private initializeSettings(
    ...
    if (!this.isSettingsInitialized) {
      this.workspaceManager =
        CordovaWorkspaceManager.getWorkspaceManagerByProjectRootPath(args.cwd);
  ...
  private launchSimulate(
    ...
    let getEditorsTelemetry = this.workspaceManager
  ...
  private changeSimulateViewport(
  ...
  private connectSimulateDebugHost(
    simulateInfo: SimulationInfo
  ): Promise<void> {
    // Connect debug-host to cordova-simulate
    let viewportResizeFailMessage = localize(
  ...
  private addBuildFlagToArgs(runArgs: Array<string> = []): Array<string> {
    const hasBuildFlag =
      runArgs.findIndex((arg) => arg.includes("--buildFlag")) > -1;
  ...
  private async launchIos(
    ...
    this.outputLogger(
      localize("LaunchingApp", "Launching the app (This may take a while)...")
    );
    ...
    const command =
      launchArgs.cordovaExecutable ||
      CordovaProjectHelper.getCliCommand(workingDirectory);
    let args = ["run", "ios"];
  ...
  private attachIos(
    ...
    return this.resolveIOSTarget(attachArgs, true).then(
      (target?: IOSTarget) => {
        if (!target) {
        ...
        const getBundleIdentifier = () => {
          return CordovaIosDeviceLauncher.getBundleIdentifier(
            attachArgs.cwd
        ...
        const getAttachRequestArgs = (): Promise<ICordovaAttachRequestArgs> =>
          CordovaIosDeviceLauncher.startWebkitDebugProxy(
            ...
            .then((iOSProcessedParams: IOSProcessedParams) => {
              attachArgs.webSocketDebuggerUrl =
                iOSProcessedParams.webSocketDebuggerUrl;
              attachArgs.iOSVersion = iOSProcessedParams.iOSVersion;
              attachArgs.iOSAppPackagePath =
                iOSProcessedParams.iOSAppPackagePath;
  ...
  private getCordovaWebview(
    ...
    let cordovaWebview = webviewsList.find((webviewData) => {
      ...
      if (this.ionicDevServerUrls) {
        return (
          this.ionicDevServerUrls.findIndex(
  ...
  private async cleanUp(restart?: boolean): Promise<void> {
    ...
    if (this.adbPortForwardingInfo) {
      ...
      adbPortPromise = this.runAdbCommand(adbForwardStopArgs, errorLogger).then(
        () => void 0
      );
    } else {
  ...
  private startIonicDevServer(
    ...
    let getRegexToResolveAppDefer = (cliArgs: string[]): RegExp => {
      ...
      let isIosDevice: boolean =
        cliArgs.indexOf("ios") !== -1 && cliArgs.indexOf("--device") !== -1;
      let isIosSimulator: boolean =
        cliArgs.indexOf("ios") !== -1 && cliArgs.indexOf("--emulator") !== -1;
      let iosDeviceAppReadyRegex: RegExp =
    ...
    const command =
      launchArgs.cordovaExecutable ||
      CordovaProjectHelper.getCliCommand(launchArgs.cwd);
    ...
    To get the list of addresses run "ionic cordova run PLATFORM --livereload" (where PLATFORM is platform name to run) and wait until prompt with this list is appeared.`
          ...
          if (addresses.length > 0) {
            ...
            errorMessage += [
              localize("AvailableAdresses", " Available addresses:"),
            ]
              .concat(addresses)
              .join(os.EOL + " ");
          }
  ...
  private async launchChromiumBasedBrowser(
    args: ICordovaLaunchRequestArgs
  ): Promise<void> {
    ...
    if (browserPath[0]) {
      ...
      this.setChromeExitTypeNormal =
        this.setChromeExitTypeNormalByUserDataDir.bind(this, args.userDataDir);
  ...
  private setChromeExitTypeNormalByUserDataDir(userDataDir: string) {
    try {
      ...
      this.outputLogger(
        "Warning: Failed to set normal exit type for Chrome browser"
      );
    }
  }
  ...
  private launchServe(
    ...
    return this.startIonicDevServer(launchArgs, args).then(
  ...
  private async resolveAndroidTarget(
    ...
    const getFirstOnlineAndroidTarget = async (): Promise<
    ...
    if (configArgs.target) {
      ...
      const isAnyEmulator =
        configArgs.target.toLowerCase() === TargetType.Emulator;
      const isAnyDevice = configArgs.target.toLowerCase() === TargetType.Device;
      const isVirtualTarget = await androidEmulatorManager.isVirtualTarget(
        configArgs.target
      );

      const saveResult = async (target: AndroidTarget): Promise<void> => {
        const launchScenariousManager = new LaunchScenariosManager(
          configArgs.cwd
        );
        if (isAttachScenario) {
      ...
      let targetDevice = await androidEmulatorManager.selectAndPrepareTarget(
        (target) => {
          const conditionForAttachScenario = isAttachScenario
            ? target.isOnline
            : true;
          const conditionForNotAnyTarget =
            isAnyEmulator || isAnyDevice
              ? true
              : target.name === configArgs.target ||
                target.id === configArgs.target;
  ...
  private async resolveIOSTarget(
    ...
    const getFirstOnlineIOSTarget = async (): Promise<
      IOSTarget | undefined
    > => {
    ...
    if (configArgs.target) {
      const isAnyEmulator =
        configArgs.target.toLowerCase() === TargetType.Emulator;
      const isAnyDevice = configArgs.target.toLowerCase() === TargetType.Device;
      const isVirtualTarget = await this.iOSTargetManager.isVirtualTarget(
        configArgs.target
      );
      ...
      let targetDevice = await this.iOSTargetManager.selectAndPrepareTarget(
        (target) => {
          const conditionForAttachScenario = isAttachScenario
            ? target.isOnline
            : true;
  ...
  private async launchAndroid(
    ...
    if (launchArgs.runArguments && launchArgs.runArguments.length > 0) {
      ...
      const targetArgs = await this.getCommandLineArgsForAndroidTarget(
        launchArgs
      );
    ...
    let cordovaResult = cordovaRunCommand(
      ...
      if (errorMatch) {
        ...
        if (!errorOutputJsonPattern.test(errorMatch[0])) {
          throw new Error(
            localize("ErrorRunningAndroid", "Error running android")
          );
        }
      }

      this.outputLogger(
        localize("AppSuccessfullyLaunched", "App successfully launched")
      );
    });
  ...
  private attachAndroid(
    ...
    let resolveTagetPromise = new Promise<string>(async (resolve, reject) => {
      try {
        const devicesOutput = await this.runAdbCommand(
          ["devices"],
          errorLogger
        );
        try {
    ...
    let packagePromise: Promise<string> = fs.promises
      .readFile(path.join(attachArgs.cwd, ANDROID_MANIFEST_PATH))
      .catch((err) => {
        if (err && err.code === "ENOENT") {
          return fs.promises.readFile(
            path.join(attachArgs.cwd, ANDROID_MANIFEST_PATH_8)
          );
        }
        throw err;
      })
    ...
    return Promise.all([packagePromise, resolveTagetPromise])
      .then(([appPackageName, targetDevice]) => {
        ...
        let getSocketsCommandArguments = [
          "-s",
          targetDevice,
          "shell",
          "cat /proc/net/unix",
        ...
        return retryAsync(
          ...
          let forwardSocketCommandArguments = [
            "-s",
            targetDevice,
            "forward",
            `tcp:${attachArgs.port}`,
          ...
          this.outputLogger(
            localize("ForwardingDebugPort", "Forwarding debug port")
          );
          return this.runAdbCommand(
            forwardSocketCommandArguments,
            errorLogger
          ).then(() => {
