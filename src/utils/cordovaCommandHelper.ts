...
export class CordovaCommandHelper {
  ...
  public static executeCordovaCommand(
    ...
    let telemetryEventName: string =
      CordovaCommandHelper.CORDOVA_TELEMETRY_EVENT_NAME;
    ...
    return CordovaCommandHelper.selectPlatform(
      projectRoot,
      command,
      useIonic
    ).then((platform) => {
            ...
            if (err.code === "ENOENT") {
              window.showErrorMessage(
                localize(
                  ...
                  "{0} not found, please run \"npm install –g {1}\" to install {2} globally",
                  cliDisplayName,
    ...
    if (cordovaDebugSession) {
      ...
      window.showErrorMessage(
        localize(
          ...
          "Cannot restart debugging of a Cordova application by the path \"{0}\". Could not find a debugging session for the application.",
          projectRoot
        )
      );
    }
  ...
  private static getSetting(fsPath: string, configKey: string): any {
    ...
    const workspaceConfiguration: WorkspaceConfiguration =
      workspace.getConfiguration("cordova", uri);
    ...
    return new Promise((resolve, reject) => {
      if (["prepare", "build", "run"].indexOf(command) > -1) {
        if (platforms.length > 1) {
          ...
          throw new Error(
            localize("NoAnyPlatformInstalled", "No any platforms installed")
          );
        }
      }
