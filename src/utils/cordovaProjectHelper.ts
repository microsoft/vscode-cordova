...
export class CordovaProjectHelper {
  ...
  public static getInstalledPlugins(projectRoot: string): string[] {
    let fetchJsonPath: string = path.resolve(
      projectRoot,
  ...
  public static getInstalledPlatforms(projectRoot: string): string[] {
    let platformsPath: string = path.resolve(
      projectRoot,
  ...
  public static getInstalledPluginDetails(
    projectRoot: string,
    pluginId: string
  ): IPluginDetails {
    let packageJsonPath: string = path.resolve(
      projectRoot,
      CordovaProjectHelper.PROJECT_PLUGINS_DIR,
      pluginId,
      "package.json"
    ...
    try {
      ...
      let details: IPluginDetails = {
        ...
        PluginType:
          CordovaProjectHelper.CORE_PLUGIN_LIST.indexOf(pluginId) >= 0
            ? "Core"
            : "Npm",
      };
  ...
  public static checkPathBelongsToHierarchy(
    parentPath: string,
    childPath: string
  ): boolean {
  ...
  public static determineIonicMajorVersion(
    ...
    if (
      fs.existsSync(
        path.join(projectRoot, CordovaProjectHelper.IONIC_PROJECT_FILE)
      )
    ) {
      return 1;
      ...
      fs.existsSync(
        path.join(projectRoot, CordovaProjectHelper.IONIC_LIB_DEFAULT_PATH)
      )
    ) {
      return 1;
    }
    ...
    if (
      ...
      (devDependencies["@ionic/app-scripts"] ||
        dependencies["@ionic/app-scripts"])
    ) {
  ...
  public static isIonicCliVersionGte(
    ...
    try {
      const ionicVersion = CordovaProjectHelper.getIonicCliVersion(
        fsPath,
        command
      );
  ...
  private static versionSatisfiesInterval(
    ...
    return (
      semver.gt(version, lowVersionEdge) && semver.lt(version, highVersionEdge)
    );
  }
  ...
  private static versionRangeSatisfiesInterval(
    ...
    return (
      semver.ltr(lowVersionEdge, version) &&
      semver.gtr(highVersionEdge, version)
    );
  }
}
