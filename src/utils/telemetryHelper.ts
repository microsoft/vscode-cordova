...
export abstract class TelemetryGeneratorBase {
  ...
  public addError(error: Error): TelemetryGeneratorBase {
    this.add(
      "error.message" + ++this.errorIndex,
      error.message,
      /*isPii*/ true
    );
    let errorWithErrorCode: IHasErrorCode = <IHasErrorCode>(
  ...
  private combine(...components: string[]): string {
    let nonNullComponents: string[] = components.filter(
...
export class TelemetryHelper {
  ...
  public static createTelemetryActivity(
    eventName: string
  ): Telemetry.TelemetryActivity {
    return new Telemetry.TelemetryActivity(eventName);
  }

  public static determineProjectTypes(
    projectRoot: string
  ): Promise<ProjectType> {
    let ionicMajorVersion =
    ...
    let mobilefirst = CordovaProjectHelper.exists(
      path.join(projectRoot, ".project")
    );
    let phonegap = CordovaProjectHelper.exists(
      path.join(projectRoot, "www", "res", ".pgbomit")
    );
    let cordova = CordovaProjectHelper.exists(
  ...
  public static prepareProjectTypesTelemetry(
    projectType: ProjectType
  ): Partial<ProjectType> {
    let relevantProjectTypes: Partial<ProjectType> = Object.entries(
      projectType
    ...
    if (relevantProjectTypes.ionicMajorVersion) {
      relevantProjectTypes[`isIonic${relevantProjectTypes.ionicMajorVersion}`] =
        true;
  ...
  public static sendPluginsList(
    ...
    let pluginDetails: IPluginDetails[] = newPlugins
      .map((pluginName) =>
        CordovaProjectHelper.getInstalledPluginDetails(projectRoot, pluginName)
      )
      .filter((detail) => !!detail);

    let pluginEvent = new Telemetry.TelemetryEvent("plugins", {
