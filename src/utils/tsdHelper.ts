...
export class TsdHelper {
  ...
  public static installTypings(
    ...
    TelemetryHelper.generate("addTypings", (generator) => {
      ...
      return Promise.all(
        typeDefsPath.map((relativePath: string) => {
          ...
          if (projectRoot) {
            ...
            let userTypingsShortPath = path.join(
              projectRoot,
              TsdHelper.USER_TYPINGS_FOLDERNAME,
              path.basename(relativePath)
            );
            let userTypingsLongPath = path.join(
              projectRoot,
              TsdHelper.USER_TYPINGS_FOLDERNAME,
              relativePath
            );
      ...
      let typingsFolder = path.resolve(
        projectRoot,
        TsdHelper.USER_TYPINGS_FOLDERNAME
      );
      ...
      let references = CordovaProjectHelper.existsSync(indexFile)
  ...
  public static removeTypings(
    ...
    let referencesToPersist = references.filter(
      (ref) =>
