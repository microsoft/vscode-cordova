...
suite("сordovaProjectHelper", function () {
  ...
  suite("isCordovaProject", function () {
    test("should return 'true' in case 'config.xml' is in a workspace root folder", () => {
      const isCordovaProject =
        CordovaProjectHelper.isCordovaProject(testProjectPath);
  ...
  suite("determineIonicMajorVersion", function () {
    const ionicProjectPath = path.join(
      __dirname,
      "..",
      "resources",
      "testIonicProject"
    );
    const ionicPackageJsonFileLocation = path.join(
      ionicProjectPath,
      "package.json"
    );
