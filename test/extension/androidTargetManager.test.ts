...
} from "../../src/utils/android/androidTargetManager";
import {
  IDebuggableMobileTarget,
  IMobileTarget,
} from "../../src/utils/mobileTarget";
...
suite("AndroidTargetManager", function () {
  ...
  suiteSetup(() => {
    ...
    getAbdsNamesStub = Sinon.stub(adbHelper, "getAvdsNames").callsFake(
      async () => {
        return [
    ...
    getOnlineTargetsStub = Sinon.stub(adbHelper, "getOnlineTargets").callsFake(
      async () => {
    ...
    launchSimulatorStub = Sinon.stub(
      ...
      "launchSimulator"
    ).callsFake(async (emulatorTarget: IMobileTarget) => {
