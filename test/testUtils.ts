...
export function executeCordovaCommand(
  cwd: string,
  command: string
): Promise<any> {
...
export function createCordovaProject(
  cwd: string,
  projectName: string
): Promise<any> {
...
export function removeCordovaComponents(
  ...
  let cordovaCommandToRun =
    componentName + " remove " + componentsToRemove.join(" ");
  return executeCordovaCommand(projectRoot, cordovaCommandToRun);
}

export function enumerateListOfTypeDefinitions(projectRoot: string): string[] {
  let typeDefsFolder =
    CordovaProjectHelper.getCordovaPluginTypeDefsPath(projectRoot);
